...
class TargetReader:
    ...
    color_steps = {'yellow': 2, 'red': 4, 'blue': 6, 'black': 8}
    orig_image = None
    image = None
    image_gray = None
    stage_images = []
    ...
    def process_frame(self, frame):
        ...
        error_message = None
        if self.remove_skew():
            error_message = 'Could not find target corners'
        elif self.standardize_size():
            error_message = 'Could not identify target'
        elif self.balance_contrast():
            error_message = 'Could not balance contrast'
        elif self.find_shots():
            error_message = 'Could not detect shots'
        elif self.get_shot_data():
            error_message = 'Could not create shot dataframe'
    ...
    def remove_skew(self):
        ...
        # Update to unpack only two values from findContours
        ...
        # Reorders corner points to Top Left, Top Right, Bottom Right, Bottom Left
    ...
    def get_shot_data(self):
        ...
        self.df = df
        return None
    # Define other methods here such as remove_skew, standardize_size, balance_contrast, find_shots, get_shot_data
    # Assuming each method processes the frame and updates self.image or other class variables.

# Main section to run the application
if __name__ == "__main__":
    reader = TargetReader()
    reader.start_video_processing()
